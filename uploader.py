...
TRAIN_COLS_OUT = ['src_ip', 'dst_ip', 'src_port', 'src_mac', 'dst_mac', 'timestamp']
...
def plot_history(data):
    ...
    fig, ax = plt.subplots()
    sns.barplot(x=label_encoder.inverse_transform(count.index), y=count.values, ax=ax)
...
if uploaded_file is not None:
    ...
    try:
        st.dataframe(latest_data[TRAIN_COLS_OUT])
    except Exception as e:
